...
struct nalu *nalu_parse_all(unsigned char *buffer, unsigned int length,
                            unsigned int *count) {
  unsigned int i;
  int nalu_start_flag;
  ...
  for (i = 0; i < length - NALU_START_CODE_SIZE; i++) {
    if ((0 == buffer[i]) && (0 == buffer[i + 1]) && (0 == buffer[i + 2]) &&
        (1 == buffer[i + 3])) {
      nalu_start_flag = TRUE;
      nalu_start_offset = i;
      i += 4;
    } else if ((0 == buffer[i]) && (0 == buffer[i + 1]) &&
               (1 == buffer[i + 2])) {
      nalu_start_flag = TRUE;
      nalu_start_offset = i;
      i += 3;
  ...
  struct nalu *nalu_array =
      (struct nalu *)malloc(nalu_count * sizeof(struct nalu));
  if (NULL == nalu_array) {
    goto END;
  }
  ...
  for (i = 0; i < nalu_count; i++) {
    ...
    // // H264
    nalu_array[i].type =
        (enum nal_unit_type)((nalu_header & 0x7F) >> 1); // H265
  }
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < nalu_count; i++) {
    printf("nalu_array[%d].type = %s, prefix = %d len = %d\n", i,
           h265_nalu_type(nalu_array[i].type), nalu_array[i].prefix,
           nalu_array[i].length);
