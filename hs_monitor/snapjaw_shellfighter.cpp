...
// 为什么会执行到这里，因为以 health 作为 key 注册的，那么肯定 evt 里面的角色
// health 是将要发生变化的
void HS_FUNC_SNAPJAW_SHELLFIGHTER(struct hs_card *card, struct hs_event *evt) {
  ...
  // tmp_card = hs_search(evt->dst, HS_ATTR_BATTLE_POS, HS_OPERATION_EQUAL,
  // left) if (card)
  ...
  // PRE FUNC 对 event 进行操作，或者产生新的 event ？反正都是对 event
  // 的添加删除和修改操作，需要相关 API POST FUNC 呢，产生新的 event
}
