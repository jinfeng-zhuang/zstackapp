...
static void calc_waveform_parameter(void) {
  ...
  app.waveform_parameter.formf =
      app.waveform_parameter.rms / app.waveform_parameter.aver;
  app.waveform_parameter.crestfpos =
      app.waveform_parameter.max / app.waveform_parameter.rms;
  app.waveform_parameter.crestfneg =
      app.waveform_parameter.min / app.waveform_parameter.rms;
  app.waveform_parameter.impulsefpos =
      app.waveform_parameter.max / app.waveform_parameter.aver;
  app.waveform_parameter.impulsefneg =
      app.waveform_parameter.min / app.waveform_parameter.aver;
  app.waveform_parameter.marginfpos =
      app.waveform_parameter.max / app.waveform_parameter.smra;
  app.waveform_parameter.marginfneg =
      app.waveform_parameter.min / app.waveform_parameter.smra;
  ...
  log_info("%15s %f\n"
           ...
           "Max", app.waveform_parameter.max, "Min", app.waveform_parameter.min,
           "Mean", app.waveform_parameter.mean, "Average",
           app.waveform_parameter.aver, "RMS", app.waveform_parameter.rms,
           "SMRA", app.waveform_parameter.smra, "FormF",
           app.waveform_parameter.formf, "CrestFPos",
           app.waveform_parameter.crestfpos, "CrestFNeg",
           app.waveform_parameter.crestfneg, "ImpulseFPos",
           app.waveform_parameter.impulsefpos, "ImpulseFNeg",
           app.waveform_parameter.impulsefneg, "MarginFPos",
           app.waveform_parameter.marginfpos, "MarginFNeg",
           app.waveform_parameter.marginfneg, "Kurtosis",
           app.waveform_parameter.kurtosis, "Skewness",
...
static int find_running_speed_true(float *power, unsigned int length,
                                   unsigned int ref_line, float *true_speed) {
  unsigned int i;
  ...
  float sum;
  float petal_power_sum, left_petal_ratio, right_petal_ratio, petal_ratio_sum,
      speed_true;
  ...
  if ((petal_ratio_sum < 0.33) || (petal_ratio_sum > 0.52)) {
    ...
    *true_speed = speed_true;
    ...
    // log_info("True speed: %f %f\n", speed_true, Amp_1xRPM_true); // TODO
    // 'Amp_1xRPM_true' not used
...
unsigned long long np_amax(enum np_type, void *a, unsigned int s,
                           unsigned int e) {
  unsigned int i;
...
unsigned long long np_amin(enum np_type, void *a, unsigned int s,
                           unsigned int e) {
  unsigned int i;
  float *a_double = (float *)a;
 ...
 * 比较，保留大的
 */
static int find_true_pkpk(float *wvfm, unsigned int length, float spd,
                          float *pkpk) {
  ...
  while (j <= (wvfm_len - 1)) {
    true_pkpk_digit = max(true_pkpk_digit, np_amax(NP_DOUBLE, wvfm, i, j) -
                                               np_amin(NP_DOUBLE, wvfm, i, j));
...
static int calculate_probability_density(float *x) { return 0; }
...
static char *para_nonsyn[] = {
    "Subsynchronous", "Nonsyn 1-2xRPM", "Nonsyn 2-3xRPM", "Nonsyn 3-4xRPM",
    "Nonsyn 4-5xRPM", "Nonsyn 5-6xRPM", "Nonsyn 6-7xRPM", "Nonsyn 7-8xRPM",
    "Nonsyn 8-9xRPM", "Nonsyn 9-10xRPM"};

static char *para_harmonic[] = {
    "Harmonic 1xRPM", "Harmonic 2xRPM", "Harmonic 3xRPM", "Harmonic 4xRPM",
    "Harmonic 5xRPM", "Harmonic 6xRPM", "Harmonic 7xRPM", "Harmonic 8xRPM",
    "Harmonic 9xRPM", "Harmonic 10xRPM"};

static int calculate_spectral_parameters(float *pw_spc, unsigned int length,
                                         float spd, float *para_overall_pkpk,
                                         float *pa_nonsyn_value,
                                         float *pa_harmonic_value) {
  float pa_span = 0.15;
  ...
  *para_overall_pkpk = overall_pkpk_value;
  ...
  // 计算频段的上下限
  pa_nonsyn_span[0][0] = (float)FRAME_SIZE * FREQ_LIMIT_LOW / SAMPLE_FREQ +
                         0.5; // TODO if float take effect
  ...
  if (pa_harmonic_span[harmonic_number - 1][1] > (spc_len - 1)) {
    ...
    pa_nonsyn_span[harmonic_number][1] =
        round((harmonic_number + 1 - pa_span) * spd);
  ...
  log_info("-------------------------------------------------------------------"
           "------------------------------------\n");
  log_info("%20s\t%4s %4s %10s %6s\n", "Parameter Name", "Bin", "Edge",
           "Microns", "Digital");
  log_info("-------------------------------------------------------------------"
           "------------------------------------\n");
  for (i = 0; i < 10; i++) {
    log_info("%20s:\t%4d %4d %10.5f %6.0f\n", para_nonsyn[i],
             pa_nonsyn_span[i][0], pa_nonsyn_span[i][1],
             digit2micron(pa_nonsyn_value[i]), pa_nonsyn_value[i]);
  }
  for (i = 0; i < 10; i++) {
    log_info("%20s:\t%4d %4d %10.5f %6.0f\n", para_harmonic[i],
             pa_harmonic_span[i][0], pa_harmonic_span[i][1],
             digit2micron(pa_harmonic_value[i]), pa_harmonic_value[i]);
  }
  log_info("-------------------------------------------------------------------"
           "------------------------------------\n");
#endif
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < SAMPLE_SIZE; i++) {
    ...
    split(digit_str, sizeof(digit_str), (char *)file_content, file_size, ',',
          &split_offset);
  ...
  for (i = 0; i < 5; i++) {
    ...
    memcpy(&app.sample[SAMPLE_CALC_ADDR], &app.sample[sample_start_point],
           FRAME_SIZE * sizeof(float));

    linear_regression(&app.sample[SAMPLE_CALC_ADDR], FRAME_SIZE);

    // TODO: shouldn't change the value in sample, otherwise the overlapped area
    // will be change
    hanning_window(&app.sample[SAMPLE_CALC_ADDR], FRAME_SIZE);

    memset(&app.sample[SAMPLE_CALC_ADDR + FRAME_SIZE], 0,
           FRAME_SIZE * sizeof(float));
    fft(&app.sample[SAMPLE_CALC_ADDR],
        &app.sample[SAMPLE_CALC_ADDR + FRAME_SIZE], FRAME_SIZE);
    power_spectrum(&app.sample[SAMPLE_CALC_ADDR],
                   &app.sample[SAMPLE_CALC_ADDR + FRAME_SIZE],
                   &app.sample[SAMPLE_CALC_ADDR], FRAME_SIZE);

    // FFT Normalization
    for (j = 1; j < FRAME_SIZE; j++) {
      app.sample[SAMPLE_CALC_ADDR + j] =
          app.sample[SAMPLE_CALC_ADDR + j] * 2.0 / FRAME_SIZE;
    }
  ...
  sum = 0;
  for (i = 0; i < app.lines; i++) {
    app.sample[ADDR_VEL + i] =
        app.sample[ADDR_FFT + i] * i * 2.0 * PI * SAMPLE_FREQ /
        FRAME_SIZE; // 将位移谱转为速度谱，谱上每一点乘以2*PI*该点频率
    sum += pow(app.sample[ADDR_VEL + i], 2);
  }
  app.vel_Overall = sum;
  app.vel_Overall = sqrt(app.vel_Overall * 8.0 / 3.0) / 2 / 2 /
                    sqrt(2.0);              // 修正，并转为有效均方根值
  ...
  sum = 0;
  for (i = 0; i < app.lines; i++) {
    app.sample[ADDR_ACC + i] =
        app.sample[ADDR_VEL + i] * i * 2.0 * PI * SAMPLE_FREQ /
        FRAME_SIZE; // 将位移谱转为速度谱，谱上每一点乘以2*PI*该点频率
    sum += pow(app.sample[ADDR_ACC + i], 2);
  }
  app.acc_Overall = sum;
  app.acc_Overall = sqrt(app.acc_Overall * 8.0 / 3.0) / 2 / 2 /
                    sqrt(2.0); // 修正，并转为有效均方根值
  ...
  find_running_speed_true(&app.sample[ADDR_POWER], app.lines, app.spd_ref_line,
                          &app.spd_true_line);
  ...
  if (app.spd_true_line > 0.0) {
    calculate_spectral_parameters(
        &app.sample[ADDR_POWER], app.lines, app.spd_true_line,
        &app.overall_pkpk, app.para_nonsyn_value, app.para_harmonic_value);
  }

  // 真峰峰值
  find_true_pkpk(&app.sample[ADDR_RAW], SAMPLE_SIZE, app.spd_true_line,
                 &app.true_pkpk_digit);
