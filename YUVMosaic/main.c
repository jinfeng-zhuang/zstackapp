  ...
  for (i = 0; i < app.param.count; i++) {
    ...
    for (y = rect->y; y < rect->y + rect->h; y++) {
      for (x = rect->x; x < rect->x + rect->w; x++) {
        TL_Y =
            app.wrap->GetY(app.input_file_content, app.param.width,
                           app.param.height, x / step * step, y / step * step);
        TL_U =
            app.wrap->GetU(app.input_file_content, app.param.width,
                           app.param.height, x / step * step, y / step * step);
        TL_V =
            app.wrap->GetV(app.input_file_content, app.param.width,
                           app.param.height, x / step * step, y / step * step);

        app.wrap->SetY(app.input_file_content, app.param.width,
                       app.param.height, x, y, TL_Y);
        app.wrap->SetU(app.input_file_content, app.param.width,
                       app.param.height, x, y, TL_U);
        app.wrap->SetV(app.input_file_content, app.param.width,
                       app.param.height, x, y, TL_V);
      }
    }

    info(".");
    ret = file_append(app.param.output_filename, app.input_file_content,
                      app.frame_size);
    if (ret != app.frame_size) {
      warn("failed\n");
      goto END;
