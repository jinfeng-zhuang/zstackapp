...
int param_parser(int argc, char *argv[], struct application *app) {
  ...
  while ((c = getopt_long(argc, argv, "", opts, NULL)) != -1) {
    ...
    case OPTION_RESOLUTION:
      ...
      break;
    case OPTION_RECT:
      ret = sscanf(optarg, "%d,%d,%d,%d", &app->param.rect.x,
                   &app->param.rect.y, &app->param.rect.w, &app->param.rect.h);
      if (4 != ret)
        return -1;
      break;
    case OPTION_COUNT:
  ...
  if (optind + 1 < argc) {
    if (strlen(argv[optind + 1]) >= FILENAME_MAX)
      return -1;
    memcpy(app->param.crop_filename, argv[optind + 1],
  ...
  if (optind + 2 < argc) {
    if (strlen(argv[optind + 2]) >= FILENAME_MAX)
      return -1;
    memcpy(app->param.output_filename, argv[optind + 2],
  ...
  if (0 == app->param.output_filename[0]) {
    ...
    snprintf(app->param.output_filename, FILENAME_MAX,
             "%d-%02d-%02d_%02d-%02d-%02d.yuv", tm.year, tm.month, tm.day,
