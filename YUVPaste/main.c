...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < app.frame_num; i++) {
    ...
    ret = file_load(app.param.crop_filename, offset_crop, app.crop_file_content,
                    app.crop_frame_size);
    ...
    for (y = rect->y; y < rect->y + rect->h; y++) {
      for (x = rect->x; x < rect->x + rect->w; x++) {
        Y = app.wrap->GetY(app.crop_file_content, rect->w, rect->h, x - rect->x,
                           y - rect->y);
        U = app.wrap->GetU(app.crop_file_content, rect->w, rect->h, x - rect->x,
                           y - rect->y);
        V = app.wrap->GetV(app.crop_file_content, rect->w, rect->h, x - rect->x,
                           y - rect->y);

        app.wrap->SetY(app.input_file_content, app.param.width,
                       app.param.height, x, y, Y);
        app.wrap->SetU(app.input_file_content, app.param.width,
                       app.param.height, x, y, U);
        app.wrap->SetV(app.input_file_content, app.param.width,
                       app.param.height, x, y, V);
      }
    }

    // TODO
    info("save frame\n");
    file_append(app.param.output_filename, app.input_file_content,
                app.frame_size);
  }
