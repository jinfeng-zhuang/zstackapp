    ...
    for (i = 0; i < app.param.count; i++) {
        ...
        for (y = rect->y; y < rect->y + rect->h; y++) {
            for (x = rect->x; x < rect->x + rect->w; x++) {
                Y = app.wrap->GetY(app.input_file_content, app.param.width, app.param.height, x, y);
                U = app.wrap->GetU(app.input_file_content, app.param.width, app.param.height, x, y);
                V = app.wrap->GetV(app.input_file_content, app.param.width, app.param.height, x, y);

                app.wrap->SetY(app.output_file_content, rect->w, rect->h, x - rect->x, y - rect->y, Y);
                app.wrap->SetU(app.output_file_content, rect->w, rect->h, x - rect->x, y - rect->y, U);
                app.wrap->SetV(app.output_file_content, rect->w, rect->h, x - rect->x, y - rect->y, V);
            }
        }

        info("save frame\n");
        file_append(app.param.output_filename, app.output_file_content, app.out_frame_size);
    }
