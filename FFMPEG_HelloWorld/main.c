...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < 20; i++) {
        ...
        info("Parser (%dx%d): pict_type = %d pts = %lld repeat_pict = %d field_order = %d, picture_structure = %d\n",
            parser->width, parser->height,
            parser->pict_type,
            parser->pts,
            parser->repeat_pict,
            parser->field_order,
            parser->picture_structure
            );
...
#if 1
        info("pict_type = %d top_field_first = %d, repeat_pict = %d, interlace = %d\n",
            frame->pict_type,
