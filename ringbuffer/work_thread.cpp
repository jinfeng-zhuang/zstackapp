...
int worker_thread(int argc, char *argv[]) {
  ...
  if (0 == ret) {
    ...
    if ((chip_id(id) != CHIP_SX7B) && (chip_id(id) != CHIP_SX7A) &&
        (chip_id(id) != CHIP_SX8B) && (chip_id(id) != CHIP_SX8A)) {
      log(LOG_WARNING, "Not supported\n");
      goto END;
    }

    ret = dbg_avmips_read32(GET_VIDEO_DEBUG_INFO_VESDESCADDR | 0 << 6,
                            &ves_addr, 1);
    if (0 != ret) {
      goto END;
    }
...
#define UXL_HW_DEMUX_REG_BASE_MASK 0xFFFFF000
    ...
    while (1) {
      if (flag_hardware_demux) {
        ...
        dbg_host_read8(ves_addr, (unsigned char *)&HWDesc,
                       sizeof(struct HWDemuxVESDesc_t));
        ...
        dbg_host_read8(ves_addr, (unsigned char *)&Desc,
                       sizeof(struct AVStreamVESDesc_t));
      }
      ...
      log(LOG_USER, "%s: %#X %#X %dMB %#X %#X, remain %dM %dK %dB, %s\n",
          flag_hardware_demux ? "HW" : "SW", Desc.start, Desc.end,
          (Desc.end - Desc.start + 1) >> 20, Desc.wp, Desc.rp,
          remain >> 20 & 0x3FF, remain >> 10 & 0x3FF, remain & 0x3FF,
          flag_dir ? "--->" : "<---");
