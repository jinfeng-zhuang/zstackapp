...
int main(int argc, char *argv[]) {
  ...
  file_load(app.param.input_filename, 0, app.input_file_content,
            wrapper->FrameSize(app.param.width, app.param.height));

  for (j = 0; j < app.param.height; j++) {
    for (i = 0; i < app.param.width; i++) {
      Y = wrapper->GetY(app.input_file_content, app.param.width,
                        app.param.height, i, j);
      U = wrapper->GetU(app.input_file_content, app.param.width,
                        app.param.height, i, j);
      V = wrapper->GetV(app.input_file_content, app.param.width,
                        app.param.height, i, j);
