...
int config_parse(struct config *config, char *buffer) {
  ...
  item =
      cJSON_GetObjectItemCaseSensitive(monitor_json, "running_threshold_digit");
  config->running_threshold_digit = item->valuedouble;

  item = cJSON_GetObjectItemCaseSensitive(monitor_json,
                                          "overload_threshold_digit");
...
int main(int argc, char *argv[]) {
  ...
  info("speed_rpm_array: %d %d %d\n", config.ref_speed_rpm_array[0],
       config.ref_speed_rpm_array[1], config.ref_speed_rpm_array[2]);
  info("interval: %f %f %f\n", config.interval[0], config.interval[1],
       config.interval[2]);
